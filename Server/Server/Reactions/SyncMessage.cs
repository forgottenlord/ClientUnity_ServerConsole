...
namespace Server.NetMessages
{
    public class SyncMessage : Message
    {
        ...
        {
            StringBuilder dataToSend = new StringBuilder("SyncMsg|").Append(units.Count);
            foreach (Unit u in units)
            {
                dataToSend.Append("|").Append(u.clientName)
                    .Append("|").Append(u.posX)
                    .Append("|").Append(u.posY)
                    .Append("|").Append(u.posZ);
            }
            server.Broadcast(sender, dataToSend.ToString());
            Console.WriteLine("\r\n SyncMsg " + sender.clientName + " request sent: " + dataToSend);
        }
